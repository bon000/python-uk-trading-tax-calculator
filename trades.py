...
class Trade(object):
    ...
    def _has_preprocess_data(self):
        return all([x in self.args_used for x in self._optional_columns_preprocess()])

    def _has_allocation_data(self):
        return all([x in self.args_used for x in self._optional_columns_allocation()]) and \
            "SignQuantity" in self.args_used
    ...
    def _check_inputs(self):
        ...
        if "SignQuantity" in self.args_used:
            if "Quantity" in self.args_used and "BS" in self.args_used:
        ...
        if "typestring" in self.args_used:
            assert self.typestring in ["Open", "Close", "OverClose"]

        if "Value" in self.args_used:
    ...
    def __init__(self, **kwargs):
        ...
        for key in kwargs:
            args_used.append(key)
            # Here we are setting some member variable names (for example "Price", "Quantity", etc)
    ...
    def modify(self, **kwargs):
        ...
        self.args_used = list(set(args_used))
        self._check_inputs()
    ...
    def add_value(self, raiseerror=True):
        if "Value" in self.args_used and raiseerror:
            raise Exception("Can't add_value on trade as Value field already set")

        if not "SignQuantity" in self.args_used:
