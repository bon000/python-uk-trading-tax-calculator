...
class TradeList(list):
    ...
    def separate_code(self):
        """
        Returns a trade_dict, with codes separated out
        """
    ...
    def _separate_fx(self):
        """
        Returns a trade_dict, with FX separated out
    ...
    def add_signed_quantities(self):
        ...
        """
        return TradeList([trade.add_signed_quantity() for trade in self if "SignQuantity" not in trade.args_used])
    ...
    def final_positions_as_dict(self):
        return self.separate_code().final_positions_as_dict()
    ...
    def add_fxdict_rates(self, fx_dict):
        ...
        """
        self = self._separate_fx().add_fx_rates(fx_dict).as_joint_list()
    ...
    def add_tradeids(self):
        existing_ids_exist = ["TradeID" in trade.args_used for trade in self]
    ...
    def pop_earliest_closing_trade(self):
        """
    ...
    def partial_pop_idx(self, tradeidx, maxtopop):
        """
...
class TradeDictByCode(dict):
    ...
    def generate_pseudo_trades(self):
        """
        Removes trades with type 'OverClose' and generates two orders in place 
        """

        [x.spawn_pseudo_trades() for x in self.values()]
        return self
